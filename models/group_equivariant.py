...
class RepCNN(NormalCNN):
    ...
    def __init__(self, n_classes=10, rep_fact=4*math.sqrt(2)):
        
        super(NormalCNN, self).__init__()
        k = 1/math.sqrt(2)
        self.conv1 = Conv2dRepeat((int(32*k), 1, 7, 7), (int(32*k*rep_fact), 1, 7, 7),stride=1, padding=1)
        self.bn1 = BatchNorm2d(int(32*k*rep_fact), eps=1e-05, momentum=0.1, affine=True, track_running_stats=True)
        self.conv2 = Conv2dRepeat((int(48*k), int(32*k), 5, 5), (int(48*k*rep_fact), int(32*k*rep_fact), 5, 5),stride=1, padding=2)
        self.bn2 = BatchNorm2d(int(48*k*rep_fact), eps=1e-05, momentum=0.1, affine=True, track_running_stats=True)
        self.conv3 = Conv2dRepeat((int(64*k), int(48*k), 5, 5), (int(64*k*rep_fact), int(48*k*rep_fact), 5, 5),stride=1, padding=2)
        self.bn3 = BatchNorm2d(int(64*k*rep_fact), eps=1e-05, momentum=0.1, affine=True, track_running_stats=True)
        self.conv4 = Conv2dRepeat((int(64*k), int(64*k), 5, 5), (int(64*k*rep_fact), int(64*k*rep_fact), 5, 5),stride=1, padding=2)
        self.bn4 = BatchNorm2d(int(64*k*rep_fact), eps=1e-05, momentum=0.1, affine=True, track_running_stats=True)
        self.conv5 = Conv2dRepeat((int(96*k), int(64*k), 5, 5), (int(96*k*rep_fact), int(64*k*rep_fact), 5, 5),stride=1, padding=2)
        self.bn5 = BatchNorm2d(int(96*k*rep_fact), eps=1e-05, momentum=0.1, affine=True, track_running_stats=True)
        self.conv6 = Conv2dRepeat((int(128*k), int(96*k), 5, 5), (int(128*k*rep_fact), int(96*k*rep_fact), 5, 5),stride=1, padding=0)
...
def c16_rep(num_classes):
    model = RepCNN(num_classes, 4*math.sqrt(2))
    return model

def c8_rep(num_classes):
    model = RepCNN(num_classes, 4)
    return model

def c4_rep(num_classes):
    model = RepCNN(num_classes, 2*math.sqrt(2))
    return model